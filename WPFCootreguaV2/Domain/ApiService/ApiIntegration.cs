        ...
        public static async Task<string> CallApiCootregua(string controller, object data)
        {
            try
            {
                ...
                using (var httpClient = new HttpClient(handler))
                {
                    ...
                    {
                        EventLogger.SaveLog(EventType.Info, "ERROR {endpoint}: ");
                        return string.Empty;
                    }
            ...
            //ErrorServicioTercero
                        case StateTransaction.ErrorServicioTercero:
                            EventLogger.SaveLog(EventType.Info, responseApi.ResponseMessage + " Cootregua");
                            break;

                        case StateTransaction.Aprobada:
                            EventLogger.SaveLog(EventType.Info, "DATA RECIBIDA ENCRYPTADA {endpoint}");

                            string dataApi = Encryptor.Decrypt(responseApi.ResponseData.ToString(), Key);
                            EventLogger.SaveLog(EventType.Info, "DATA RECIBIDA DESENCRYPTADA {endpoint}: " + dataApi);
                            return dataApi;
                        default:
                            break;
                    }
            ...
            {
                EventLogger.SaveLog(EventType.Info," ApiIntegration");
            }
